...
- 状态：Active
- 当前阶段：第 11 步九布局原生形状渲染已完成；下一步将 DeepSeek 对齐视觉 DeckSpec
- 更新时间：2026-09-22
...
- [x] 使用 `python-pptx` 为九种布局实现确定性原生对象绘制，并通过 PowerPoint 导出九页真实预览。
...
- [x] 提取统一画布、安全边距、字体、颜色和层级原语。
- [x] 实现文字、圆角卡片、色块、圆形节点、连接线、箭头和强调数字的共享绘制函数。
- [x] 为九种布局实现确定性排版和容量处理，不接受模型坐标。
- [x] 保证所有布局函数不修改原始 `DeckSpec`。
- [x] 保存后使用 `python-pptx` 重新打开，检查页数、主要文字、形状类型和对象数量。
- [x] 生成九布局示例 PPTX，并用 PowerPoint 导出 9 张 1920×1080 PNG 完成人工检查。
...
- [x] 更新 `prompts/plan_deck.md`，要求先规划故事线，再按内容从九种布局中选择视觉结构。
- [x] 明确数字、时间轴、流程、对比和双栏的选择规则，并禁止虚构数字、图片、来源、自由坐标与绘图代码。
- [x] 使用九布局 fixture 和 FakeProvider 覆盖普通测试，不访问网络。
- [x] 在 Provider 内使用兼容端点可接受的扁平传输 schema，剔除空字段后仍由带 discriminator 的领域 `DeckSpec` 执行严格二次校验。
- [x] 对模型偶发的重复 `slide_id` 按页面顺序添加确定性后缀；领域层仍拒绝未经 Provider 规范化的重复 ID。
- [x] 按布局展示指标、时间节点、步骤、对比内容和双栏摘要。
- [x] 保持“未确认不渲染；重新规划不复用旧候选；失败不覆盖成功产物”。
- [x] 显式运行真实 `deepseek-v4-flash` 测试；6、8、10 页样例均页数准确、ID 唯一，并按语义选择多种布局；真实 Streamlit 规划进入 `outline` 且确认前未生成文件。
...
- DeepSeek 的版式选择仍受用户描述清晰度和模型稳定性影响；非法或不完整结果会被 `DeckSpec` 二次校验拒绝。
